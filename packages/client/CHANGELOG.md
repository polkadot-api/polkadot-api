...
- BREAKING: `Transaction<Pallet, Name, Asset, Ext>` removed pallet and name parameters to `Transaction<Asset, Ext>`.
- BREAKING: Drop support for CommonJS
...
- BREAKING: Remove exported `TxCall`, `TxObservable`, `TxPromise` and `TxSign` interfaces, inlined directly into `Transaction`
