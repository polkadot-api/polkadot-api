...
export async function generate(opts: GenerateOptions) {
  ...
  const replaced = await replacePackageJson(descriptorsDir, hash)
...
async function replacePackageJson(descriptorsDir: string, version: bigint) {
  const filename = join(descriptorsDir, "package.json")
  if (existsSync(filename)) {
    const parsed = parsePackage(await fs.readFile(filename, "utf-8"))
    if (parsed.version.endsWith(`.${version}`)) {
      return false
    }
  }
