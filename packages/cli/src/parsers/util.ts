...
export async function composeSourceMaps(formerMap: any, latterMap: any) {
  if (formerMap) {
    if (!latterMap) {
      return formerMap
    }
  ...
  const smcFormer = await new SourceMapConsumer(formerMap)
  const smcLatter = await new SourceMapConsumer(latterMap)
