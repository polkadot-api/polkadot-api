...
describe("storage", () => {
  it("subscribes and decodes storage values", async () => {
    const client = createClient(getForkliftProvider("storage_sub"))
  ...
  it("entries decodes storage keys", async () => {
    const client = createClient(getForkliftProvider("storage_ent"))
    const api = client.getTypedApi(paseo)
  ...
  it("returns the raw key if the hasher is opaque", async () => {
    const client = createClient(getForkliftProvider("storage_raw"))
    const api = client.getTypedApi(paseo)
